...
import QueryModal from "../Misc/QueryModal";

const InterestedRes = ({ item }) => {
    ...
    <div className="rounded-lg px-6 xs:px-3 xs:py-3 bg-[#000]/40 py-4 ">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center space-x-4">
          <div onClick={() => setModal(true)} className="text-white cursor-pointer py-2 px-4 border border-[#787878] rounded-lg ">
            Send Query
          </div>
        </div>
      </div>
      <QueryModal isOpen={modal} onClose={() => setModal(false)} />
    </div>
  );
};

export default InterestedRes;
